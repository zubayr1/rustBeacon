...
fn communicate_to_client(mut stream: TcpStream, sk: String) {
    ...
    while match stream.read(&mut data) {
        Ok(size) => {
            ...
            let sk1 = sk.clone();
            let plaintext = create_sign(sk1)+ " "+ "Hello from node2";
...
fn handle_server(node2_port: u32, sk: String) {
...
//init function
pub fn initiate_node2(random_number: u32, node_port_start: u32, pubkeys:&mut Vec<String>) {

    let mut file = File::open("./node2_sk.txt").expect("cant open the file");
    ...
    if random_number%4==1
    {
        ...
        let handle4 = thread::spawn(move || {
            ...
            handle_server(node_port_start+2+4, sk3);
    ...
    {                   
        ...
        let handle4 = thread::spawn(move || {            
            ...
            handle_client(node_port_start+2+4, pubkeys3);
...
pub fn create_keys()
{
    ...
    println!("node2 {:?}", keypair);
    println!("node2 {:?}", signature);
